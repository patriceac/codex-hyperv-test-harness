...
Begin with read-only inspection and an explicit source/public audit. Run `setup\Update-RequestNetworking.ps1 -InstallRoot '<EXACT_INSTALL_ROOT>' -PolicyPath '<LOCAL_UNTRACKED_POLICY_JSON>' -PlanOnly`, report its fingerprinted exact plan and every unresolved elevated identity check, and stop for a separate approval. `-PlanOnly` must not elevate, write files, create or change switches/NAT/firewall/VM adapters, restart the broker, run application canaries, or refresh recovery. After the second approval, use the same policy file and approved fingerprint; if any host identity or fingerprint changed, stop without mutation and renew review. A successful live update must drain the broker, preserve a rollback copy, deploy the broker policy transactionally, run positive and negative canaries for every enabled profile, prove cancellation/orphan cleanup and final worker disconnection, run the pool audit, refresh and deep-verify local recovery, and keep any public push as a separately approved source-only action.
