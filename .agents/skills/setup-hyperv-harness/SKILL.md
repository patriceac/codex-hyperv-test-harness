...
For an ordinary committed software release to an existing installation, use `setup\Deploy-HarnessRelease.ps1` and `docs\deployment.md` as the canonical workflow. Run its immutable PlanOnly result first, report the exact deployment ID, plan hash, component readiness, guest-baseline scope, `RecoveryBaselineExportMode`, `RecoveryReuseReadiness`, absence or presence of a real live shadow pool, acceptance matrix, and one-final-recovery policy, then obtain approval before Apply. Clear ordinary wording that refers to the displayed plan is sufficient approval; do not require a magic phrase. `ReuseCurrent` is valid only when the plan reports no guest-baseline change and Current has a matching successful receipt; it must require the same VM/checkpoint identity, reuse baseline files by verified same-volume NTFS hard links, hash the non-baseline delta, and stop instead of silently falling back to `FullExport`. First recovery creation and every real baseline/checkpoint change use `FullExport`. Let the controller own source qualification, source publication, any guest-baseline/pool replacement, isolated legacy/accented-name/keyboard/expected-power-off acceptance, and one final recovery refresh. Resume valid checkpoints instead of repeating successful expensive work. For an ordinary source failure prefer a linked fix-forward plan; seek separate authority for rollback, ForceRebuild, image servicing, networking changes, or any expanded destructive scope. Update the controller, documentation, and deterministic contract test together whenever this topology changes.

After intentional baseline or backend changes, run `setup\Refresh-LocalRecovery.ps1 -InstallRoot '<USER_SELECTED_NON_ROOT_DIRECTORY>'`; its direct default is `FullExport`, and a baseline-changing workflow must never pass `ReuseCurrent`. Before any public push, rerun `setup\Test-PublicRepository.ps1`; no media, VM state, binaries, credentials, request evidence, or personal profile data may enter Git.
