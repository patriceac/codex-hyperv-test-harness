...
$removalAst = (Get-Command Remove-SystemPromptQueryUserBlockRulesV1).ScriptBlock.Ast.Find({
    param($node)
    $node -is [Management.Automation.Language.ScriptBlockExpressionAst]
}, $true)
Assert-True ($null -ne $removalAst) 'The guest-side Query User removal script block was not found.'
$removalBody = $removalAst.ScriptBlock.Extent.Text
$removeQueryBlocks = [scriptblock]::Create($removalBody.Substring(1, $removalBody.Length - 2))
& {
    # Exercise the actual guest code without accessing the host firewall.
    $storedRules = New-Object Collections.Generic.List[object]
    $storedRules.AddRange([object[]]@($expectedAllow, $tcpQueryBlock, $udpQueryBlock, $unrelatedBlock))
    function Get-NetFirewallRule {
        [CmdletBinding()]
        param([string] $PolicyStore, [string] $Name)
        Assert-True ($PolicyStore -ceq 'PersistentStore') 'Query User removal read the wrong policy store.'
        $storedRules | Where-Object { [string]::IsNullOrEmpty($Name) -or [string]$_.Name -ceq $Name }
    }
    ...
    function Remove-NetFirewallRule {
        ...
        process { Assert-True ($storedRules.Remove($InputObject)) 'Removal did not target an existing exact rule.' }
    }
    $removedRules = @(& $removeQueryBlocks $exactExecutable $queryBlocksJson)
    Assert-True ($removedRules.Count -eq 2 -and
        [string]$removedRules[0].Name -ceq $tcpQueryBlock.Name -and [string]$removedRules[0].Protocol -ceq 'TCP' -and
        [string]$removedRules[1].Name -ceq $udpQueryBlock.Name -and [string]$removedRules[1].Protocol -ceq 'UDP') 'Query User removal did not independently resolve and remove the TCP and UDP rules.'
    Assert-True ($storedRules.Count -eq 2 -and $storedRules.Contains($expectedAllow) -and $storedRules.Contains($unrelatedBlock)) 'Query User removal changed an allow rule or an unrelated application rule.'
}
$scenarios.Add('firewall-query-user-tcp-udp-removal')
