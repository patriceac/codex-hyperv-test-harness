...
Assert-Rejected -Scenario 'old operation cannot request connectivity' -ExpectedMessage 'use RunGuestJobNetworkV1' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        Operation = 'RunGuestJob'
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'IsolatedTestNet'; Cohort = 'same-run'; SwitchName = $null; AllowHostInputs = $false }
...
Assert-Rejected -Scenario 'new operation cannot downgrade to none' -ExpectedMessage 'requires an explicit non-None' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        ...
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'None'; Cohort = $null; SwitchName = $null; AllowHostInputs = $false }
    }) -Config $config | Out-Null
}
...
$isolated = Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
    Operation = 'RunGuestJobNetworkV1'
    HostInputs = @()
    Network = [pscustomobject]@{ Profile = 'IsolatedTestNet'; Cohort = 'contract.test-42'; SwitchName = $null; AllowHostInputs = $false }
}) -Config $config
...
Assert-Rejected -Scenario 'raw network field' -ExpectedMessage 'unsupported properties' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        Operation = 'RunGuestJobNetworkV1'
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'IsolatedTestNet'; Cohort = 'safe'; SwitchName = $null; AllowHostInputs = $false; DnsServers = @('8.8.8.8') }
    }) -Config $config | Out-Null
}
Assert-Rejected -Scenario 'truthy string is not acknowledgement' -ExpectedMessage 'exact string, null, and Boolean JSON types' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        ...
        Network = [pscustomobject]@{ Profile = 'IsolatedTestNet'; Cohort = 'safe'; SwitchName = $null; AllowHostInputs = 'false' }
    }) -Config $config | Out-Null
}
...
Assert-Rejected -Scenario 'disabled InternetOnly' -ExpectedMessage 'disabled by the SYSTEM broker policy' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        Operation = 'RunGuestJobNetworkV1'
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'InternetOnly'; Cohort = $null; SwitchName = $null; AllowHostInputs = $false }
    }) -Config $config | Out-Null
}
Assert-Rejected -Scenario 'disabled TrustedLan' -ExpectedMessage 'disabled by the SYSTEM broker policy' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        Operation = 'RunGuestJobNetworkV1'
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'TrustedLan'; Cohort = $null; SwitchName = 'External'; AllowHostInputs = $false }
    }) -Config $config | Out-Null
}
...
Assert-Rejected -Scenario 'TrustedLan missing physical pin' -ExpectedMessage 'missing its broker-pinned' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        Operation = 'RunGuestJobNetworkV1'
        HostInputs = @()
        Network = [pscustomobject]@{ Profile = 'TrustedLan'; Cohort = $null; SwitchName = 'External'; AllowHostInputs = $false }
...
Assert-Rejected -Scenario 'network plus share always rejected' -ExpectedMessage 'cannot coexist' -Operation {
    Resolve-RequestNetworkProfile -Request ([pscustomobject]@{
        ...
        Network = [pscustomobject]@{ Profile = 'IsolatedTestNet'; Cohort = 'safe'; SwitchName = $null; AllowHostInputs = $true }
    }) -Config $config | Out-Null
}
