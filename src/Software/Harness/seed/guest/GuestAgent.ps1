...
public static class CodexGuestInput
{
    ...
    private static bool IsExtendedKey(ushort virtualKey)
    {
        return virtualKey == 0x21 || virtualKey == 0x22 || virtualKey == 0x23 || virtualKey == 0x24 ||
            ...
            virtualKey == 0x2D || virtualKey == 0x2E || virtualKey == 0x5B;
    }

    public static void SendKeyChord(ushort[] virtualKeys, int holdMilliseconds)
    {
        INPUT[] down = new INPUT[virtualKeys.Length];
        INPUT[] up = new INPUT[virtualKeys.Length];
        for (int index = 0; index < virtualKeys.Length; index++)
        {
            down[index].type = INPUT_KEYBOARD;
            down[index].data.keyboard.virtualKey = virtualKeys[index];
            down[index].data.keyboard.flags = IsExtendedKey(virtualKeys[index]) ? KEYEVENTF_EXTENDEDKEY : 0;

            int reverseIndex = virtualKeys.Length - index - 1;
            up[index].type = INPUT_KEYBOARD;
            up[index].data.keyboard.virtualKey = virtualKeys[reverseIndex];
            up[index].data.keyboard.flags = KEYEVENTF_KEYUP | (IsExtendedKey(virtualKeys[reverseIndex]) ? KEYEVENTF_EXTENDEDKEY : 0);
        }

        uint pressed = SendInput((uint)down.Length, down, Marshal.SizeOf(typeof(INPUT)));
        int pressError = pressed == down.Length ? 0 : Marshal.GetLastWin32Error();
        if (pressed == down.Length)
        {
            System.Threading.Thread.Sleep(holdMilliseconds);
        }
        uint released = SendInput((uint)up.Length, up, Marshal.SizeOf(typeof(INPUT)));
        if (pressed != down.Length)
        {
            throw new System.ComponentModel.Win32Exception(pressError, "SendInput did not press the complete key chord.");
        }
        if (released != up.Length)
        {
            throw new System.ComponentModel.Win32Exception(Marshal.GetLastWin32Error(), "SendInput did not release the complete key chord.");
        }
    }
...
function Get-ValidatedKeyChord {
    ...
    foreach ($key in $keys) {
        if (-not $virtualKeys.ContainsKey($key)) { throw "$Context send_keys key '$key' is not supported." }
    }
            ...
            try {
                switch ($actionType) {
                    ...
                    'send_keys' {
                        ...
                        $chord = Get-ValidatedKeyChord -Action $action -Context "Action $actionIndex"
                        Focus-Window -WindowHandle $windowHandle
                        [CodexGuestInput]::SendKeyChord([uint16[]]$chord.VirtualKeys, [int]$chord.HoldMilliseconds)
                        Start-Sleep -Milliseconds 250
                        $actionDetails = [ordered]@{
                            KeySpec = [string]$chord.KeySpec
                            Keys = @($chord.Keys)
                            VirtualKeyCodes = @($chord.VirtualKeys | ForEach-Object { '0x{0:X2}' -f [int]$_ })
                            HoldMilliseconds = [int]$chord.HoldMilliseconds
                            TargetWindowHandle = $windowHandle.ToInt64()
                        }
                    }
                    'click_relative' {
