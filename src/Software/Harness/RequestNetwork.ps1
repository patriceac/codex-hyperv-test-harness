...
function Get-RequestNetworkLeaseInventory {
    ...
    @(
        Get-ChildItem -LiteralPath $leaseRoot -Filter '*.json' -File -ErrorAction Stop | ForEach-Object {
            ...
            try { Get-Content -Raw -LiteralPath $leasePath -ErrorAction Stop | ConvertFrom-Json -ErrorAction Stop }
...
function Get-RequestNetworkInternetVlanSettings {
    ...
    foreach ($property in @('PrimaryVlanId', 'SecondaryVlanId')) {
        ...
        try {
            $numeric = [int]$value
            if ([double]$numeric -ne [double]$value) { throw 'not an integer' }
        }
        catch { throw "InternetOnly.$property must be an integer VLAN ID." }
        if ($numeric -lt 1 -or $numeric -gt 4094) {
            throw "InternetOnly.$property must be between 1 and 4094."
        }
        if ($value -is [bool] -or [string]::IsNullOrWhiteSpace([string]$value)) {
            throw "InternetOnly.$property must be an integer VLAN ID."
        }
    }
...
function Resolve-RequestNetworkProfile {
    ...
    if ($network) {
        $allowedProperties = @('Profile', 'Cohort', 'SwitchName', 'AllowHostInputs')
        ...
        $switchValue = Get-RequestNetworkObjectPropertyValue -Value $network -Name 'SwitchName'
        ...
        if ($profileValue -isnot [string] -or ($null -ne $cohortValue -and $cohortValue -isnot [string]) -or
            ($null -ne $switchValue -and $switchValue -isnot [string]) -or $allowValue -isnot [bool]) {
    ...
    $switchName = if ($network) { [string]$switchValue } else { '' }
    ...
    if ($profile -eq 'None') {
        if (-not [string]::IsNullOrWhiteSpace($cohort) -or -not [string]::IsNullOrWhiteSpace($switchName) -or $allowHostInputs) {
            throw 'The None network profile cannot include cohort, switch, or host-input acknowledgement fields.'
        }
    ...
    switch ($profile) {
        'IsolatedTestNet' {
            ...
            if (-not [string]::IsNullOrWhiteSpace($switchName)) { throw 'IsolatedTestNet does not accept a client-supplied switch.' }
            $null = Get-RequestNetworkIPv4Prefix24 -Prefix ([string]$settings.NetworkPrefix) -Context 'IsolatedTestNet.NetworkPrefix'
        }
        'InternetOnly' {
            if (-not [string]::IsNullOrWhiteSpace($cohort) -or -not [string]::IsNullOrWhiteSpace($switchName)) {
                throw 'InternetOnly does not accept a client-supplied cohort or switch.'
            }
        ...
        'TrustedLan' {
            ...
            if ($switchName.Length -lt 1 -or $switchName.Length -gt 128 -or $switchName.IndexOfAny([char[]](0..31)) -ge 0) {
                throw 'TrustedLan requires a bounded switch selector without control characters.'
            }
            $approved = @($settings.AllowedSwitches | Where-Object { [string]::Equals([string]$_.Name, $switchName, [StringComparison]::Ordinal) })
            if ($approved.Count -ne 1) { throw "TrustedLan switch '$switchName' is not uniquely allowlisted by broker policy." }
...
function Get-RequestNetworkExpectedNatPolicy {
    ...
    foreach ($bound in @(
        ...
        $value = [long](Get-RequestNetworkObjectPropertyValue -Value $Settings -Name $bound.Name)
        if ($value -lt $bound.Minimum -or $value -gt $bound.Maximum) { throw "InternetOnly.$($bound.Name) is outside its broker safety bound." }
    }
    [pscustomobject][ordered]@{
...
function New-RequestNetworkRuntime {
    ...
    elseif ($profile -eq 'TrustedLan') {
        $approved = @($Definition.Settings.AllowedSwitches | Where-Object { [string]::Equals([string]$_.Name, [string]$Definition.RequestedSwitchName, [StringComparison]::Ordinal) })
...
function Prepare-RequestVmNetwork {
    ...
    Import-Module Hyper-V -ErrorAction Stop
    Remove-ManagedRequestNetworkAdapters -VmName $VmName | Out-Null
...
function Remove-ManagedRequestNetworkAdapters {
    ...
    [pscustomobject][ordered]@{ Success = $true; Errors = @() }
}
...
function Recover-OrphanedRequestNetworkResources {
    ...
    foreach ($vmName in @(Get-RequestNetworkManagedVmNames -BrokerRoot $BrokerRoot)) {
        foreach ($adapter in @(Get-VMNetworkAdapter -VMName $vmName -ErrorAction Stop | Where-Object { $_.Name -like 'CodexRequestNet-*' })) {
            $leased = @($remainingLeases | Where-Object { $_ -and [string]::Equals([string]$_.VmName, $vmName, [StringComparison]::Ordinal) -and [string]::Equals([string]$_.AdapterName, [string]$adapter.Name, [StringComparison]::Ordinal) })
            if ($leased.Count -gt 0) { continue }
            try {
