...
- After the verified application window appears, a cooler-violet rounded outline identifies the controlled target. Its crisp two-pixel core and restrained inward fade stay inside the visible window bounds, follow movement and resizing, and remain click-through and non-activating. It hides whenever another application owns the foreground, so it never stays painted over an unrelated foreground window. Maximized or edge-to-edge windows use square inner corners so the outline remains visible without spilling off-screen.
