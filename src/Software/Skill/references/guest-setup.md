...
Guest setup permits only `None` and `IsolatedTestNet`. It cannot be combined with read-only host inputs or expected guest power-off. It may be combined with explicit system-prompt acceptance for the normal application; the setup executable itself never receives prompt handling. Setup elevation comes only from the already protected SYSTEM broker's administrator guest session, and all changes disappear with the disposable OS child.
