...
Before input, the controller retains hash-verified handles for both executables and requires a fresh native UAC event naming the original executable and a live observed requester in its process tree. The event emitter must be the exact signed consent process in the initiating session. If Windows defers the prompt on the normal desktop, the handler may send a restore command to the single visible native interim window owned by that exact consent process. Message delivery alone does not establish prompt readiness. Credentials and decisions still require the attributed consent process in the secure desktop foreground and positively identified controls. Unknown, stale, ambiguous or mismatched prompts fail closed. Acceptance requires a separately observed elevated instance with the expected administrator SID; decline requires no such instance. This contract covers self-elevation of the same executable, not arbitrary child installer packages, auto-elevation, additional prompts or installer-triggered reboots.

The event must be fresh when the prompt binding is established. That binding has a fixed deadline: `PromptTimeoutSeconds` from establishment, capped by the remaining request deadline. Continuing the same prompt does not renew it. Exact live process/creation/session identities, the bound image handle, secure foreground, and the original window/control identities remain required before input. `SecureUiProgress` contains only allowlisted phase names and UTC timestamps, including helper initialization; entering a phase does not prove its input completed. When no final decision receipt exists, `Input` remains null and credential/decision outcomes are unknown. Progress checkpoints never establish `ContractProven`.

The host also supervises the controller's exact process creation identity and fixed phase deadline. Initialization allows 120 seconds, account preparation and the intentional reboot allow 300 seconds each, and desktop, verifier and prompt phases use their bounded deadlines; every phase remains capped by the request deadline. An exited controller without completion is reported promptly. `installer-status.json` retains the last successfully observed, allowlisted status separately from the latest probe outcome and timestamp. Safe phase, secure-progress and final decision summaries are also written to the outbox as they occur. Installer failures use the bounded 30-second cleanup-only evidence transfer into `failure-diagnostics` before VM disposal, preserving available setup logs and verifier output. Those partial files never become a successful terminal receipt or a product verdict; unavailable diagnostics and missing decisions remain unknown.
