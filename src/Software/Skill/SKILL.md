...
- `IsolatedTestNet` requires `-NetworkCohort <NON_SECRET_LABEL>`. Only concurrent requests in the same explicitly named cohort may share its private VM-only switch; the broker activates an exact request-adapter/subnet-bound guest ingress rule, and the switch has no host, LAN, or Internet route.
