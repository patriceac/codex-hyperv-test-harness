...
- `IsolatedTestNet` requires `-NetworkCohort <NON_SECRET_LABEL>`. Only concurrent requests in the same explicitly named cohort may share its private VM-only switch; it has no host, LAN, or Internet route.
