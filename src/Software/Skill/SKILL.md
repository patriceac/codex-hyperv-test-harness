...
The host controller gives a six-second thin, continuous fuchsia halo warning on every display before launch. After the verified application window appears, a separate cooler-violet outline identifies that exact controlled window with a crisp two-pixel core and restrained inward fade; its bands move as one atomic batch, recalculate shape only when geometry changes, dim while paused, disappear when the window is unavailable, and hide while another application owns the foreground. Its UI thread establishes `PerMonitorV2` before creating any overlay so scaled displays use the same physical-pixel coordinates as DWM; failure is terminal. Only one host controller may own the current interactive session at a time. Ordinary mouse or keyboard activity is ignored for the first three seconds. During the final three seconds it pauses the warning countdown immediately and turns the screen halo amber; after launch it pauses automation the same way. Control resumes only after ten seconds without user input, returning to the remaining warning countdown or restoring the controlled application to the foreground first. Physical `Escape` cancels at any time. Both visual guards are click-through, non-activating, and excluded from evidence capture. These protections do not make host execution isolated.
