...
Baseline promotion sets and verifies non-expiring account/password policy only for the disposable guest automation account. Guest readiness attests that policy before accepting work. A legacy baseline whose password already expired must first complete Windows' password-change flow using the protected existing credential while the controller waits for PowerShell Direct; preserve that credential. The controller then seals the corrected baseline, replaces the workers, and performs one full recovery refresh. This does not alter host account policy.
