...
Deterministic source tests must cover the `None` compatibility default, `RunGuestJobNetworkV1` for every non-`None` request, runner schema validation, authoritative broker allowlisting, rejection of raw network configuration, cohort and switch constraints, host-input `Share` rejection, `Auto`-to-VHDX selection, lease-before-connect ordering, connect-last/disconnect-first lifecycle reporting, cancellation, retry, and orphan reconciliation. They must also prove that terminal result publication follows verified adapter cleanup and that an old broker rejects rather than downgrades a network request.
