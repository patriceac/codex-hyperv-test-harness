...
For external monitoring, poll the queue JSON once a minute and alert on `PlatformStatus = Degraded`, including `HealthReasons`; treat planned `Maintenance` separately. Keep the eleven isolated release checks as the deployment gate.
