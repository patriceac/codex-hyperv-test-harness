...
Queue status retries brief state-file sharing violations. If the pool snapshot remains unreadable, capacity is unknown and `PoolStateUnavailable` is reported without inferring orphaned work or capacity violations.
...
For external monitoring, poll the queue JSON once a minute and alert on `PlatformStatus = Degraded`, including `HealthReasons`; treat planned `Maintenance` separately. Keep the twelve isolated release checks as the deployment gate.
