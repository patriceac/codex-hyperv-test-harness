...
The local recovery bundle exports the exact verified baseline and installed source. GitHub stores only the rebuild and servicing logic, so a cold rebuild resolves then-current official Windows media, applicable non-preview updates, and the pinned stable .NET SDK rather than reproducing a byte-identical historical image.
