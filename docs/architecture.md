...
An explicit user-authorized physical-host run follows a separate, unprivileged path and does not enter this broker or pool. The runtime skill launches an on-demand controller in the current interactive user session, establishes a `PerMonitorV2` physical-pixel coordinate space before WinForms creates its first window, uses the shared action vocabulary, surrounds every display with one thin, continuous, non-activating fuchsia frame, and adds a cooler-violet rounded inward outline to the verified controlled window once it appears. Both overlays are click-through and follow display or target-window geometry changes without DPI virtualization drift; the controlled-window outline is visible only while that application owns the foreground, and the controller stops rather than falling back when focus or capture safety cannot be verified. The initial unpaused warning countdown lasts six seconds: physical input is ignored for the first three and pauses control during the final three. `Escape` cancels at any time. This convenience path intentionally does not claim any of the disposable-guest boundaries shown above.
