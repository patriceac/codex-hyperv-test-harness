...
The optional host runner is an on-demand process in the current interactive user session, not a SYSTEM desktop bridge. It requires an internal authorization switch that Codex passes only after the explicit named override. A five-second fuchsia halo precedes launch on every display. Low-level keyboard and mouse hooks classify physical input separately from marked controller-generated `SendInput`; physical activity pauses the action sequence, ten idle seconds permit resumption only after verified application refocus, and physical `Escape` cancels. Halo windows are click-through and non-activating. The runner requests OS capture protection, records whether it was available, and keeps its own screenshots clean by copying only the controlled app bounds with a brief halo-only suppression when full exclusion is unavailable.
