...
- `IsolatedTestNet` joins only an explicit test cohort on a private VM-only switch. It must not route to the host, LAN, or Internet.
...
Every non-`None` request uses `RunGuestJobNetworkV1`, so an older broker fails closed. The client cannot authorize itself by naming a switch: profile, cohort, and switch selectors are matched against private broker configuration, and raw switch/NAT/DNS/route/firewall objects are rejected. Broker validation is authoritative even when the runtime skill already validated the request.

User-profile integration is deliberately outside the privileged phase. The exact target user's unelevated process installs the runtime skill and managed Codex policy through a same-parent staged replacement that rejects reparse points, alternate data streams, source changes, incomplete copies, and incomplete rollback. Elevation receives only content fingerprints; after source is staged under the protected installation root, the privileged controller recomputes those fingerprints without reading, mirroring into, or executing from the user-controlled profile tree.
