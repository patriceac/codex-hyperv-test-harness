...
For an existing installation, use the resumable [harness release controller](docs/deployment.md) instead of manually chaining source refresh, guest-baseline replacement, acceptance, and recovery commands. It binds a clean commit and installed configuration into an immutable PlanOnly result and applies through one elevated state machine. Its eleven isolated acceptance paths cover installer self-elevation, standard-user administrator credentials, explicit UAC decline, legacy launch, an accented UI Automation `Name` click without an `AutomationId`, screenshot-backed `WIN+LEFT` keyboard input, expected guest power-off with no replay, executable-bound UAC plus Windows Firewall prompt handling, automatic then manual sign-in across restarts with traffic to a separate guest after both boots, installed-app shutdown, and diagnostic retention after a deliberate restart-phase failure.
