...
When the user explicitly overrides isolation for one named artifact and test, the installed runtime skill also contains an on-demand physical-host controller. It gives a five-second fuchsia halo warning on every display, pauses on physical mouse or keyboard activity, resumes after ten idle seconds only after refocusing the tracked application, and lets physical `Escape` cancel. This path is never an automatic fallback and provides no VM, filesystem, credential, network, or rollback isolation; see the runtime skill's [host-control contract](src/Software/Skill/references/host-control.md).
