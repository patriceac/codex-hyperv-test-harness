...
| [Guarded physical-host control](src/Software/Skill/references/host-control.md) | Only an explicit override for one named artifact/test enables the unprivileged interactive-host runner under current PowerShell 7/.NET or Windows PowerShell 5.1. Its fuchsia screen warning, inward violet controlled-window outline, physical-input pause/resume, `Escape` cancellation, focus verification, evidence, and process cleanup reduce accidental interference but provide none of the VM isolation guarantees. |
...
When the user explicitly overrides isolation for one named artifact and test, the installed runtime skill also contains an on-demand physical-host controller. It gives a six-second thin, continuous fuchsia halo warning on every display; once the verified application window appears, a cooler-violet rounded outline follows that window and fades inward without covering its controls. The outline hides whenever another application owns the foreground, so it cannot remain painted over an unrelated foreground window. The visual-guard thread enters Windows `PerMonitorV2` mode before creating any overlay, keeping DWM window bounds and halo placement in the same physical-pixel coordinate space at scaled display settings such as 125%. Ordinary mouse or keyboard activity is ignored for the first three seconds; during the final three seconds it pauses the countdown and turns the screen halo amber. Control resumes after ten idle seconds, returning to the remaining warning countdown or refocusing the tracked application as appropriate. Physical `Escape` cancels at any time. This path is never an automatic fallback and provides no VM, filesystem, credential, network, or rollback isolation; see the runtime skill's [host-control contract](src/Software/Skill/references/host-control.md).
