...
| [Simultaneous VM groups](src/Software/Skill/references/request-groups.md) | A group queues until every member and its full worker count are available. Atomic admission prevents partial groups from occupying the pool while waiting for more VMs; cancellation and failures stop unfinished peers. |
...
| [Request-scoped guest setup](src/Software/Skill/references/guest-setup.md) | One exact payload-manifest-bound executable may run through the guest administrator session before the normal medium-integrity application. Commands and product assertions stay in the application test; setup evidence and disposable-VM cleanup remain generic. |
| [Installer UAC](src/Software/Skill/references/installer-uac.md) | Directly launched medium-integrity installers support attributed self-elevation, standard-user administrator credential entry, and explicit UAC decline. Exact process identities and fixed controller and prompt deadlines bound the workflow. Failed runs attempt bounded diagnostic collection before VM disposal; missing final input receipts remain unknown. This mode disables live capture. |
| [Live and terminal evidence](src/Software/Skill/references/queue-observation-and-cancellation.md) | Terminal results separate harness success from application-test success and record action, process-cleanup, payload, VM, network, and evidence state. Eligible running requests can provide a fresh screenshot plus explicitly allowlisted small `{OUTDIR}` files without changing their deadline or substituting stale evidence. |
...
Non-`None` requests use the versioned `RunGuestJobNetworkV1` operation so an older broker rejects them instead of silently running with different connectivity. Clients may select only an enabled profile and, for `IsolatedTestNet`, a cohort; they cannot select a switch or supply raw NAT, DNS, route, or firewall configuration. `TrustedLan` resolves the policy's sole pinned external switch entirely inside the broker. See [request networking](docs/request-networking.md) for the fingerprinted infrastructure and broker-policy deployment workflows, and the runtime skill's [network and host-input reference](src/Software/Skill/references/network-and-host-inputs.md) for request examples.

This source contract does not enable or create live profile infrastructure. Adding switches, NAT, firewall policy, or allowlists remains part of the setup-harness workflow: create the exact fingerprinted plan, then apply it automatically when ready under standing authorization. See [request-scoped networking](docs/request-networking.md) for the exact request contract, lifecycle, preflight, and live acceptance matrix.
...
>
> - Installation directory — suggested layout: `<user-selected-large-local-fixed-drive>:\VMs\Codex-Harness`, with at least 200 GiB free. Do not assume that `D:` or any other drive exists. If I do not know which drive to use, perform a read-only fixed-drive and free-space inventory before recommending one.
...
> - Preservation — suggested: preserve every existing VM and file and keep `ForceRebuild = false`; when replacement is required by this requested rebuild, verify the exact managed assets and proceed under standing authorization.
...
>
> After I answer, present an exact proposed configuration, paths, resource totals, named machine changes, downloads, security boundaries, destructive possibilities, and verification plan. All operations within this requested harness workflow are authorized by default; do not pause for confirmation.
>
> Run the repository's public-safety audit and `-PlanOnly` preflight using every chosen value explicitly. Show me the resulting plan and any discrepancy. If it succeeds and remains within this requested workflow, continue immediately through elevation, Hyper-V enablement, ISO or large-file download, reboot, scheduled-task registration, VM or VHDX creation/replacement, broker installation, Codex policy/skill change, verification, recovery refresh, and source publication without asking again. Never configure Windows activation or licensing.
...
For an existing installation, use the resumable [harness release controller](docs/deployment.md) instead of manually chaining source refresh, guest-baseline replacement, acceptance, and recovery commands. It binds a clean commit and installed configuration into an immutable PlanOnly result and applies through one elevated state machine. Its twelve isolated acceptance paths cover installer self-elevation, standard-user administrator credentials, explicit UAC decline, legacy launch, an accented UI Automation `Name` click without an `AutomationId`, screenshot-backed `WIN+LEFT` keyboard input, expected guest power-off with no replay, executable-bound UAC plus Windows Firewall prompt handling, automatic then manual sign-in across restarts with traffic to a separate guest after both boots, installed-app shutdown, diagnostic retention after a deliberate restart-phase failure, and competing VM groups admitted together.

Source qualification also proves that a BOM-less PowerShell 7 request containing accented text is read correctly by Windows PowerShell 5.1. The controller refreshes and verifies local recovery once at the end; when the canonical checkpoint is unchanged, it reuses the receipt-backed baseline export with NTFS hard links and writes only the software delta. Image servicing, networking changes, first installation, and `ForceRebuild` remain separate reviewed workflows.

## Reference profile and compatibility defaults

Codex-guided recovery must obtain every required configuration value. The entries below describe the project's established profile and the fallback retained for manual script compatibility; execution remains standing-authorized once those choices are known.
...
| Temporary guest-update switch | Configured `Default Switch` |
