...
| [Guarded physical-host control](src/Software/Skill/references/host-control.md) | Only an explicit override for one named artifact/test enables the unprivileged interactive-host runner under current PowerShell 7/.NET or Windows PowerShell 5.1. Its fuchsia warning frame, physical-input pause/resume, `Escape` cancellation, focus verification, evidence, and process cleanup reduce accidental interference but provide none of the VM isolation guarantees. |
